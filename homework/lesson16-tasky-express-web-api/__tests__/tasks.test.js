...
test('Update a task', async () => {
    const task = await tasks.insertTask('rambo', 2, 'run-shoes', 'Buy new running shoes')
    await tasks.updateTask('rambo', task.id, 3, 'shark suit', 'Memorise the full song, and put the suit')
    const actual = await tasks.getTask('rambo', task.id)
    expect(actual.title).toBe('shark suit')
    expect(actual.description).toBe('Memorise the full song, and put the suit')
    await tasks.deleteTask('rambo', task.id)
    const allAfter = await tasks.getAll('rambo')
    expect(allAfter.length).toBe(1)
})